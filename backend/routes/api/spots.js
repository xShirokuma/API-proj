...
router.post("/:id/bookings", requireAuth, async (req, res, next) => {
  ...
  if (!spot) {
    ...
    for (const booking of spotBookings) {
      let err;
      if (
        startDateTyped.getTime() >= booking.startDate.getTime() &&
        startDateTyped.getTime() <= booking.endDate.getTime()
      ) {
        err = Error(
      ...
      if (
        endDateTyped.getTime() >= booking.startDate.getTime() &&
        endDateTyped.getTime() <= booking.endDate.getTime()
      ) {
        err = Error(
          "Sorry, this spot is already booked for the specified dates"
        );
    ...
    newBookingJson.startDate = newBookingJson.startDate
      .toISOString()
      .slice(0, 10);
...
router.get("/:id/reviews", async (req, res, next) => {
  ...
  for (const review of reviews) {
    ...
    if (!reviewJson.ReviewImages) reviewJson.ReviewImages = [];
...
router.get("/current", requireAuth, async (req, res, next) => {
  ...
  for (const userSpot of userSpots) {
    ...
    if (previewImg) userSpotJson.previewImage = previewImg.url;
    else userSpotJson.previewImage = null;
...
router.get("/", async (req, res, next) => {
  ...
  for (const spot of spots) {
    ...
    if (previewImg) spotJson.previewImage = previewImg.url;
    else spotJson.previewImage = null;
